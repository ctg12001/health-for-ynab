...
const Budget = (props: { ynabAPI: api }) => {
  ...
  const getAccounts = useCallback(async () => {
    ...
    const accounts = await ynabAPI.accounts
  ...
  const getMonthSummaries = useCallback(async () => {
    ...
    const monthSummaries = await ynabAPI.months
  ...
  const getTransactions = useCallback(async () => {
    ...
    const transactions = await ynabAPI.transactions
  ...
  const getMissingMonthDetails = useCallback(
    async (monthsToFetch: MonthSummary[]) => {
      ...
      await asyncForEach(monthsToFetch, async (month) => {
        const monthResponse = await ynabAPI.months.getBudgetMonth(
          budgetId,
          utils.convertFromISODateString(month.month)
        );
