...
export const calculateRetirementContributions = (
  ...
  _getRetirementTransactions(accounts, transactions, startDate, endDate)
    .filter((transaction) => {
      return (
        transaction.payee_name &&
        transaction.payee_name !== "Investment Returns"
      );
    })
    .forEach((transaction) => {
...
export const calculateRetirementTransfers = (
  accounts: Map<string, Account>,
  transactions: TransactionDetail[],
  startDate: Date,
  endDate: Date
...
const _getRetirementTransactions = (
  accounts: Map<string, Account>,
  transactions: TransactionDetail[],
  startDate: Date,
  endDate: Date
  ...
  const retirementAccountIDs = filterMapValues(accounts, (account) => {
    return (
      account.type === Account.TypeEnum.OtherAsset &&
      !account.name.includes("ESPP")
    );
  }).map((account) => {
    return account.id;
  });
  ...
  return transactions.filter((transaction) => {
    ...
    return (
      retirementAccountIDs.includes(transaction.account_id) &&
